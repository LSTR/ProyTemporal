...
<title>Sistema de venta pollos .::Mabel::.</title>
		...
		<table id="tablaDetalle">
			...
			<tbody id="bodyTablaDetalle">
				...
				<tr>
					...
					<?php
						$rsTPollo = $tipoPollo->getTipoPollo();
						if ($rsTPollo->num_rows) {
							while ($rowTPollo = $rsTPollo->fetch_assoc()) {
					?>
							<option value="<?php echo $rowTPollo['idTipoPollo']; ?>">
							<?php echo $rowTPollo['nombreTipoPollo']?>
							</option>
					<?php
							}
						}
				...
				elseif (@$_GET['venta']) {
					...
					if ($rsDV->num_rows) {
						...
						while ($rowDV = $rsDV->fetch_assoc()) {
							$idTipoPollo = $rowDV['idTipoPollo'];
							//$nombreTipoPollo = $rowDV['nombreTipoPollo'];
							$cantidadTipoPollo = $rowDV['cantidad'];
							$pesoNetoTipoPollo = $rowDV['pesoNeto'];
				?>
					<tr>
						<td><select name="tipoPollo[]" id="tipoPollo">
						<?php
						...
						?>
								<option value="<?php echo $rowTPollo['idTipoPollo']; ?>"
								<?php echo $rowTPollo['idTipoPollo'] == $idTipoPollo ? 'selected="selected"' : ""; ?>>
								<?php echo $rowTPollo['nombreTipoPollo']?>
								</option>
						<?php
								}
						...
						id="cantidadDet" value="<?php echo $cantidadTipoPollo; ?>" />
						</td>
						
						<td><input class="masPequenio" type="text" name="pesoDet[]"
						id="pesoDet" value="<?php echo $pesoNetoTipoPollo; ?>" />
						</td>
				...
				<?php
					...
					if ($rsDDV->num_rows) {
						while ($rowDDV = $rsDDV->fetch_assoc()) {
							$idTipoPollo = $rowDDV['idTipoPollo'];
							$cantidadTipoPollo = $rowDDV['cantidad'];
							$pesoTipoPollo = $rowDDV['peso'];
					...
					<tr>
						...
						<?php
							...
							if ($rsTPollo->num_rows) {
								while ($rowTPollo = $rsTPollo->fetch_assoc()) {
									$idTPollo = $rowTPollo['idTipoPollo'];
									$nombreTPollo = $rowTPollo['nombreTipoPollo'];
						?>
								<option value="<?php echo $idTPollo; ?>"
								<?php echo $idTPollo == $idTipoPollo ? 'selected="selected"' : ""; ?>>
								<?php echo $nombreTPollo; ?>
								</option>
						<?php
								}
							}
						...
						<td><input class="masPequenio" type="text" name="pesoDet[]"
						id="pesoDet" value="<?php echo $pesoTipoPollo; ?>" />
						</td>
