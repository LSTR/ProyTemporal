...
<div id="centralPanel">
	<h3 class="centrarText">Gesti&oacute;n Empleados</h3>
	<?php
	if ($rs->num_rows) {
	?>
	...
	<?php
		while ($row = $rs->fetch_assoc()) {
			...
			$nombres = $generales->configuraNombres($row['apellidos'], $row['nombres']);
			$dni = $row['dni'];
			$sueldo = $row['sueldo'];
			$direccion = $row['direccion'];
			$telefono = $row['telefono'] ? $row['telefono'] : "-";
			$movil = $row['movil'] ? $row['movil'] : "-";
			$rpm = $row['rpm'] ? $row['rpm'] : "-";
	?>
			<tr>
		...
		<form action="ingresoEmpleado.php" method="post" id="fIngEmpleado">
			...
			<legend>Datos nuevo empleado</legend>
				...
				<div class="claveValor">
					...
					<input type="text" name="direccion" id="direccion" maxlength="8" class="campoAncho" />
				</div>
				<div class="claveValor">
